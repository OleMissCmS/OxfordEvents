...
def google_link(title: str, start_iso: str, end_iso: str, details: str, location: str) -> str:
    return "https://calendar.google.com/calendar/render?" + urlencode({
        "action":"TEMPLATE","text":title or "Event",
        "dates":f"{_fmt(start_iso)}/{_fmt(end_iso or start_iso)}",
        "details":details or "","location":location or ""})
...
def build_ics(title: str, start_iso: str, end_iso: str, details: str, location: str) -> str:
    ...
    return "\r\n".join(["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//OxfordEvents//EN",
                          "BEGIN:VEVENT",f"SUMMARY:{title}",f"DTSTART:{s}",f"DTEND:{e}",
                          f"LOCATION:{location or ''}",f"DESCRIPTION:{details or ''}",
